...
exports.addNewSemiCategory = async (req, res) => {
  ...
  try {
    ...
    if (categoryFoundSerial) {
      return res.status(400).json({ message: "המספר סריאלי קיים במערכת." });
    }
    const categoryFoundName = await semiCategoryService.findSemiCategoryByName(
      checkName
    );
    if (categoryFoundName) {
      return res.status(400).json({ message: "השם של הקטגוריה קיים במערכת." });
    }

    category = semiCategoryService.addSemiCategory({
      checkSerialNumber,
      checkName,
    });
...
exports.getSemiCategoryProducts = async (req, res) => {
  ...
  try {
    ...
    semiCategory = await semiCategoryService.findSemiCategoryById(checkSemiId);
